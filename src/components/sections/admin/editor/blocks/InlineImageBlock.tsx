...
export default function InlineImageBlock({
  ...
  const { upload, isUploading, uploadError } = useUploadImage(
    campId,
    (newUrl) => {
  ...
  return (
    <div className="w-full flex flex-col gap-4 bg-white p-5 md:p-6 rounded-2xl border border-gray-100 transition-colors shadow-sm group/image">
      ...
      {imageUrl ? (
        ...
        <div className="w-full aspect-[4/3] max-h-[300px] rounded-2xl bg-gray-50 border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-400 gap-4 mt-2">
          ...
          <span className="font-montserrat text-sm font-medium text-center px-6">
            Wybierz zdjęcie, aby wstawić je w treść
          </span>
        </div>
      )}
      ...
      />
      {uploadError && (
        <span className="text-red-500 font-montserrat text-xs px-2">
          {uploadError}
        </span>
      )}
    </div>
