# Moduł: Uczestniczki Wyjazdu (Lista + Profil)
...
- Headline: counter "X kobiet na liście" + chip kontekstu wyjazdu.
...
export interface ParticipantListRow {
  ...
  depositAmount: number;       // Trip.deposit
...
export async function getParticipantsList(tripId: string) {
  ...
  return bookings.map(toParticipantListRow(trip));
}
```
...
```ts
function toParticipantListRow(trip: { price: Decimal; deposit: Decimal }) {
  return (b: BookingRaw): ParticipantListRow => {
    ...
    const paidAmount =
      (b.depositPaidAt ? Number(trip.deposit) : 0) +
      (b.remainderPaidAt ? Number(trip.price) - Number(trip.deposit) : 0);
...
export async function getParticipantDetail(tripId: string, bookingId: string) {
  const trip = await prisma.trip.findUnique({
    where: { id: tripId },
    select: { id: true, price: true, deposit: true },
  });
  ...
  return toParticipantDetail(trip, b);
}
```
...
export default async function Page({
  ...
  return <ParticipantsListClient tripId={id} rows={rows} />;
}
```
...
- Lista: `unstable_cache` z tagiem `trip:${tripId}:participants`, TTL 30s.
- Profil: tag `booking:${bookingId}` — zmiana statusu / dopłaty wywołuje `revalidateTag` w odpowiednich Server Actions.
- Server Actions, które dotykają tych widoków: `confirmDepositPaid`, `confirmRemainderPaid`, `sendPaymentReminder`, `manualAddBooking`, `markCheckIn` (synchronizuje również z dashboardem wyjazdu — patrz `[id]/README.md`).

### 3.7 Dotykane modele

| Model              | Po co                                                                |
| ------------------ | -------------------------------------------------------------------- |
| `Trip`             | Cena bazowa + zadatek (potrzebne do wyliczenia % i `paidAmount`)     |
...
| `TripService`      | Nazwa, czas trwania w UI                                             |
