...
export function useTripSeoForm(tripId: string | null) {
  ...
  // Auto-uruchom analizę przy pierwszym załadowaniu wyjazdu.
  useEffect(() => {
  ...
  const generateSeo = useCallback(async () => {
    ...
    try {
      ...
      if (tripId) {
        try {
          const persistRes = await fetch(`/api/admin/wyjazdy/${tripId}/seo`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const saveSeo = useCallback(
    async (redirect = true) => {
      ...
      setIsSaving(true);
      try {
        const res = await fetch(`/api/admin/wyjazdy/${tripId}/seo`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            metaTitle,
        ...
        if (redirect)
          router.push(`/admin/wyjazdy/dodaj/podsumowanie?id=${tripId}`);
      } catch (err) {
  ...
  // Bierze aktualną analizę + bieżące pola SEO + treść wyjazdu i prosi AI o wersję,
