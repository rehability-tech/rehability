# 📋 Checklist: Blokada publikacji Wyjazdu (Brak bloku opcji wyjazdowej)

**Cel:** Zapobieganie publikacji wyjazdu (`status: "PUBLISHED"`), gdy opcja wyjazdowa (np. `allowBringFriend` / toggle wyjazdu) jest włączona w ustawieniach, ale nie ma odzwierciedlenia w zbudowanych blokach (`blocks` JSON) na stronie Wyjazdu.
...
- [ ] **Lokalizacja endpointu:** Otworzyć odpowiedni plik trasy API (np. `app/api/admin/wyjazdy/[id]/route.ts` lub akcję serwerową).
