    ...
    startTransition(async () => {
      try {
        if (entryMode === "EVENT") {
          ...
          const url =
            isEdit && initial.id
              ? `/api/admin/wydarzenia/${tripId}/harmonogram/${initial.id}`
              : `/api/admin/wydarzenia/${tripId}/harmonogram`;
          ...
          const res = await fetch(`/api/admin/wydarzenia/${tripId}/slots`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                  ...
                  {services.length === 0 ? (
                    <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl">
                      <p className="text-[12px] font-medium text-amber-800 leading-relaxed">
                        To wydarzenie nie ma jeszcze zdefiniowanych usług. Dodaj
                        usługi w edytorze treści wydarzenia albo użyj „Wolnego
                        bloku”.
                      </p>
                    </div>
                  ) : (
