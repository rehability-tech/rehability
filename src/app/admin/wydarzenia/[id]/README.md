# Dashboard Wydarzenia — Premium SaaS Edition
...
└── _components/
    ├── TripDashboardMobile.tsx   ("use client", animacje motion)
    └── TripDashboardDesktop.tsx  ("use client", animacje motion)
```
...
#### a) Bazowy trip + zliczenia (1 zapytanie z relacjami)
...
```ts
const booked = trip.bookings.filter(b => b.status === "CONFIRMED").length;
const depositsPaid = trip.bookings.filter(b => b.depositPaidAt).length;
const remaindersPaid = trip.bookings.filter(b => b.remainderPaidAt).length;
const missingPayments = booked - depositsPaid;

const totalCollected = trip.bookings.reduce((sum, b) => {
  const deposit = b.depositPaidAt ? Number(trip.deposit) : 0;
  const remainder = b.remainderPaidAt ? Number(trip.price) - Number(trip.deposit) : 0;
  return sum + deposit + remainder;
}, 0);

const filledHealth = trip.bookings.filter(b => b.user?.healthProfile?.id).length;

const extrasOrders = trip.bookings.flatMap(b => b.serviceOrders);
...
```ts
const now = new Date();
const pendingActions = trip.bookings.flatMap(b => {
  const out = [];
  if (!b.depositPaidAt) out.push({ ...mkAction(b), issue: "Brak wpłaty zadatku", severity: "high" });
  if (b.depositPaidAt && !b.remainderPaidAt) out.push({ ...mkAction(b), issue: "Brak dopłaty końcowej", severity: daysUntil(trip.startDate) < 7 ? "high" : "mid" });
...
```ts
const allergyAlerts = trip.bookings
...
const [recentOrders, recentBookings, recentInvitations] = await Promise.all([
  ...
  prisma.booking.findMany({
    where: { tripId: id, depositPaidAt: { not: null } },
    orderBy: { depositPaidAt: "desc" },
    take: 8,
    select: { id: true, name: true, depositPaidAt: true },
  }),
  prisma.booking.findMany({
    where: { tripId: id, invitedById: { not: null } },
    orderBy: { createdAt: "desc" },
    take: 5,
...
import { getTripDashboardData } from "./_data/getTripDashboardData";
import TripDashboardMobile from "./_components/TripDashboardMobile";
import TripDashboardDesktop from "./_components/TripDashboardDesktop";

export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const data = await getTripDashboardData(id);
  if (!data) notFound();

  return (
    <>
      <TripDashboardMobile data={data} />
      <TripDashboardDesktop data={data} />
    </>
  );
}
```

`getTripDashboardData(id)` to czysta funkcja serwerowa zwracająca `TripDashboardData` (interfejs powyżej). Dwa komponenty kliencie dostają **ten sam payload** — gwarantuje to spójność widoku mobile vs desktop.

### Krok 5 — Cache / Rewalidacja

- Główny loader: `unstable_cache` z tagiem `trip:${id}:dashboard`, TTL 30s.
- Każdy Server Action zmieniający stan (`markCheckIn`, `confirmPayment`, `createServiceOrder`) wywołuje `revalidateTag('trip:'+id+':dashboard')`.
...
| Model            | Cel                                                  |
| ---------------- | ---------------------------------------------------- |
| `Trip`           | Podstawowe info + capacity + price/deposit           |
...
| `TripService`    | Nazwa usługi w feedzie                               |
| `TripEvent`      | (rezerwa) — gdy podepniemy "Teraz/Następnie"         |
...
1. Wydzielić `TripDashboardMobile.tsx` i `TripDashboardDesktop.tsx` z obecnego `page.tsx`.
2. Napisać `_data/getTripDashboardData.ts` z zapytaniami z sekcji 2.
