...
export default function CampShopPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`/api/admin/wydarzenia/${id}/sklep?t=${Date.now()}`, {
        cache: "no-store",
      });
      ...
      console.error(err);
      toast.error("Nie udało się wczytać sklepu wydarzenia.");
    } finally {
      setIsLoading(false);
    }
  }, [id]);
  ...
  const handleAdd = async (extraServiceIds: string[]) => {
    const res = await fetch(`/api/admin/wydarzenia/${id}/sklep`, {
      method: "POST",
    ...
    toast.success(
      json.added > 0
        ? `Dodano ${json.added} ${json.added === 1 ? "usługę" : "usług"} do wydarzenia.`
        : "Wybrane usługi są już w wydarzeniu.",
    );
    await fetchData();
  };

  // Utworzenie nowej usługi globalnej + dodanie do wydarzenia
  const handleCreate = async (payload: {
    ...
    // od razu dodajemy nową usługę do tego wydarzenia
  ...
  const handleSave = async (
    ...
    toast.success(
      scope === "global"
        ? `Zaktualizowano w katalogu i ${json.propagatedTo} wydarzeniach.`
        : "Zaktualizowano usługę w tym wydarzeniu.",
    );
    await fetchData();
    return true;
  };

  // Usunięcie usługi z wydarzenia
  const handleDelete = async (service: CampService) => {
    if (!window.confirm(`Usunąć usługę "${service.name}" z tego wydarzenia?`)) return;
    const res = await fetch(
      `/api/admin/wydarzenia/${id}/sklep?serviceId=${service.id}`,
      { method: "DELETE" },
    );
    if (!res.ok) {
      const e = await res.json().catch(() => null);
    ...
    toast.success("Usunięto usługę z wydarzenia.");
    await fetchData();
  };
  ...
  if (!data) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 text-center">
        ...
        <button
          type="button"
          onClick={() => router.push(`/admin/wydarzenia/${id}`)}
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 flex flex-col gap-8">
        ...
        <header className="relative overflow-hidden rounded-[28px] rounded-tr-none p-6 sm:p-8 shadow-[0_18px_50px_-20px_rgba(3,63,99,0.45)] border border-white/20">
          ...
          <div className="relative z-10 flex flex-col gap-5">
            ...
            >
              <ArrowLeft size={14} weight="bold" /> Pulpit wydarzenia
            </button>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="font-jakarta text-2xl sm:text-[32px] font-bold text-white leading-tight drop-shadow-sm">
                  Sklep / Usługi wydarzenia
                </h1>
                <p className="text-[13px] text-white/70 font-medium mt-1">
                  Zarządzaj usługami SPA tego wydarzenia i ich sprzedażą.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="font-jakarta text-lg font-bold text-brand-secondary">
                {view === "camp" ? "Usługi w tym wydarzeniu" : "Katalog globalny"}
            ...
            <div className="inline-flex rounded-2xl bg-white/60 backdrop-blur-sm border border-white/60 p-1 shadow-sm shrink-0">
              ...
              >
                <House size={15} weight="bold" /> To wydarzenie
              </button>
              <button
                type="button"
