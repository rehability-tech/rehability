...
import {
  ...
  homeVenueMapUrl,
  mentionsHomeVenue,
} from "@/lib/trips/homeVenue";

registerLocale("pl", pl);

/** „29 sierpnia 2026" — podpowiedź pod terminem zapisów. */
...
function BasicDataFormContent() {
  ...
  useEffect(() => {
    if (editId) {
      const fetchCampData = async () => {
        ...
        try {
          const response = await fetch(`/api/admin/wydarzenia/${editId}`);
          ...
          console.error(error);
          toast.error("Nie udało się załadować danych wydarzenia.");
        } finally {
  ...
  const handleAiSubmit = async (prompt: string, modelType: string) => {
    ...
    try {
      ...
      // Termin zapisów: AI zwraca go tylko wtedy, gdy organizator o nim wspomniał.
      // Pilnujemy, żeby nie wypadł po dacie rozpoczęcia (zapis i tak by to odrzucił).
      if (data.registrationDeadline) {
        const deadline = new Date(data.registrationDeadline);
        const start = data.startDate ? new Date(data.startDate) : null;
        if (!Number.isNaN(deadline.getTime())) {
          setRegistrationDeadline(
            start && deadline > start ? start : deadline,
          );
        }
      ...
      // Wydarzenie "u nas" — podstawiamy dane gabinetu razem z mapą, żeby nie
      // trzeba było ręcznie wyszukiwać wciąż tego samego adresu w Prudniku.
      // Nie nadpisujemy mapy, jeśli została już wcześniej podpięta.
      if (mentionsHomeVenue(prompt) || mentionsHomeVenue(data.locationName)) {
        setLocationName(HOME_VENUE.name);
        setLocationCity(HOME_VENUE.city);
        setMapUrl((prev) => prev || homeVenueMapUrl());
      }
  ...
  const handleSaveAndNext = async () => {
    ...
    try {
      const response = await fetch("/api/admin/wydarzenia/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id: editId,
      ...
      router.push(
        `/admin/wydarzenia/dodaj/edytor-tresci?id=${result.tripId || editId}`,
      );
    } catch (error) {
      const errorMessage =
    ...
    <div className="animate-in fade-in duration-500 relative">
      <div className="mb-6">
        <h2 className="text-xl font-jakarta font-bold text-[#0B3B4C]">
          {editId ? "Edytuj dane podstawowe" : "Dane podstawowe wydarzenia"}
        </h2>
        <p className="text-sm text-gray-500 font-montserrat mt-1">
          {editId
            ? "Zaktualizuj główne informacje o wydarzeniu."
            : "Wprowadź główne informacje o organizowanym wydarzeniu lub pozwól, by AI zrobiło to za Ciebie."}
        </p>
      </div>
      ...
      <form className="flex flex-col gap-10 relative z-0">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {/* Okno zapisów trzymamy tuż pod datami wydarzenia — to ta sama
                decyzja („kiedy wydarzenie" → „do kiedy przyjmujemy zapisy"). */}
            <FormDatePicker
              label="Zapisy przyjmujemy do (włącznie)"
              ...
              helperText={
                registrationDeadline
                  ? `Formularz rezerwacji działa do końca dnia ${formatDeadlineLabel(registrationDeadline)} (23:59). Później wydarzenie zostaje widoczne, ale bez zapisów.`
                  : "Zostaw puste, aby przyjmować zapisy do końca dnia rozpoczęcia wydarzenia."
              }
            />
        ...
        {/* SEKCJA 1.5: ZAPISY (RĘCZNE ZAMKNIĘCIE) */}
        <section>
          <h3 className="text-[13px] font-bold text-gray-400 uppercase tracking-wider mb-4 border-b border-gray-100 pb-2">
            Zapisy
          </h3>
          ...
          <div
            className={`border rounded-[16px] p-5 md:p-6 transition-all duration-300 ${
              registrationClosed
        ...
        {/* SEKCJA 2: LOKALIZACJA WYDARZENIA (NOWA) */}
        <section>
          <h3 className="text-[13px] font-bold text-gray-400 uppercase tracking-wider mb-4 border-b border-gray-100 pb-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <FormInput
              ...
              autoComplete="off"
              isLoading={isGeneratingData}
              helperText="Ta nazwa będzie wielką czcionką na kartach wydarzenia."
            />
            ...
            <FormLocationInput
              ...
              helperText={
                mapUrl
                  ...
                  : "Wyszukaj to miejsce, aby klienci widzieli precyzyjną mapę w szczegółach wydarzenia."
              }
            />
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <FormInput
              label="Cena całkowita wydarzenia (PLN)"
              required
              type="number"
              min="0"
              step="0.01"
          ...
          {/* PAKIET WYDARZENIE WE DWOJE */}
          {/* OPCJA WYDARZENIE WE DWOJE */}
          <div
            ...
            {allowBringFriend && editId && !hasBookingOptionsBlock && (
              <div className="mt-4 flex items-start gap-2.5 rounded-[12px] border border-amber-300 bg-amber-100/60 px-3.5 py-3 text-[12.5px] text-amber-800 font-montserrat leading-relaxed">
                ...
                <span>
                  ...
                  razie publikacja wydarzenia będzie zablokowana.
                </span>
              </div>
            )}
          </div>
      ...
      <div className="flex items-center gap-4 relative z-10">
        ...
        <div>
          ...
          <p className="text-[13px] text-gray-500 font-montserrat mt-0.5 max-w-md leading-relaxed">
            ...
            uzupełni kluczowe dane wydarzenia.
          </p>
        </div>
      </div>
      <button
