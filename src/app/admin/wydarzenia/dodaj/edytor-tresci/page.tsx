...
function ContentEditorFormContent() {
  ...
  } = useTripAiGenerator(updateField, imgPicker.pickImagesFor, campData);
  ...
  return (
    ...
    >
      ...
      <BlogCoverPicker
        ...
        uploadEndpoint={`/api/admin/wydarzenia/${editId}/upload`}
      />
      ...
      <div className="flex justify-between max-[720px]:flex-col max-[720px]:text-center max-[720px]:mb-6">
        <div className="mb-6">
          <h2 className="text-[22px] md:text-[26px] font-jakarta font-bold text-[#0B3B4C]">
            Kreator: Edytor treści wydarzenia
          </h2>
          <p className="text-[14px] text-gray-500 font-montserrat mt-1">
            Krok 2/3. Opowiedz o wydarzeniu i zbuduj stronę z przygotowanych
            modułów.
          </p>
        </div>
        <EditorToolbar
      ...
      <div className="flex flex-col-reverse sm:flex-row items-center sm:justify-between gap-3 pt-6 mt-8 border-t border-gray-200">
        <Link
          href={`/admin/wydarzenia/dodaj/dane-podstawowe${editId ? `?id=${editId}` : ""}`}
