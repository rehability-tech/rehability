...
function PodsumowanieContent() {
  ...
  const [trip, setCamp] = useState<any>(null);
  const [mainTab, setMainTab] = useState<"dane" | "design">("dane");

  useEffect(() => {
    if (!id) {
      toast.error("Brak identyfikatora wydarzenia. Powrót do listy.");
      router.push("/admin/wydarzenia/lista");
      return;
    }
    ...
    const fetchCampData = async () => {
      try {
        const response = await fetch(`/api/admin/wydarzenia/${id}`, {
        ...
        console.error(error);
        toast.error("Nie udało się załadować podsumowania wydarzenia.");
      } finally {
        setIsLoading(false);
      }
    };
