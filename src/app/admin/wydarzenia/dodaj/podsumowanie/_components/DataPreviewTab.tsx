...
export default function DataPreviewTab({ trip }: DataPreviewTabProps) {
  ...
  if (trip?.location) {
    try {
      ...
      // Fallback dla starych wydarzeń zapisanych jako zwykły tekst
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm flex items-start gap-4">
          ...
          <div className="flex flex-col">
            ...
            {/* Okno zapisów — admin widzi je zanim opublikuje wydarzenie. */}
            <p
              className={`font-montserrat text-xs font-semibold mt-2 ${
                trip?.registrationClosed ? "text-amber-600" : "text-brand-primary"
              }`}
            >
              {trip?.registrationClosed
                ? "Zapisy zamknięte ręcznie"
                : trip?.registrationDeadline
                  ? `Zapisy do ${formatReadableDate(trip.registrationDeadline)} (włącznie)`
                  : "Zapisy do dnia rozpoczęcia"}
            </p>
          </div>
        </div>
