...
interface PreviewUpcomingTripsProps {
  trip: any;
...
export default function PreviewUpcomingCamps({
  ...
  // Podtytuł bierzemy z danych wydarzenia (generuje go Architekt AI w kroku treści).
  // Bez fallbacku z konkretną obsadą — zdanie znika, gdy wydarzenie nie ma podtytułu.
  const subtitle =
    typeof trip?.subtitle === "string" ? trip.subtitle.trim() : "";
  ...
  let locationCity = "Miejsce wydarzenia";
  ...
  if (trip?.location) {
    try {
      ...
      locationCity = parsedLocation.city || "Miejsce wydarzenia";
    } catch (e) {
  ...
  const INFO_CIRCLES = [
    ...
    },
    {
      ...
      label: "Cena",
      value: `od ${formatPrice(trip?.price)} zł`,
    },
  ];
              ...
              <div className="flex justify-center gap-1.5">
                {INFO_CIRCLES.slice(0, 2).map((info, i) => (
                  ...
                  >
                    ...
                    <span className="font-montserrat font-bold text-white text-[12px] leading-tight px-1">
                      {info.value}
                    </span>
                  </div>
                ))}
            ...
            >
              {INFO_CIRCLES.map((info, i) => (
                ...
                >
                  ...
                  <span className="font-montserrat font-bold text-white leading-tight px-1 text-[13px]">
                    {info.value}
                  </span>
                </div>
              ))}
