...
function ZaproszeniaContent() {
  ...
  const [loadedSubject, setLoadedSubject] = useState(
    "Zaproszenie na wspólne wydarzenie Rehability ✈️",
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Krok istnieje wyłącznie dla opcji "zabierz osobę towarzyszącą".
        // Po jej wyłączeniu strona jest wciąż osiągalna z historii przeglądarki
        // albo z linku — wtedy przenosimy od razu do kolejnego kroku.
        if (!data.allowBringFriend) {
          toast.info(
            'Krok "E-mail" jest dostępny tylko przy włączonej opcji "Zabierz osobę towarzyszącą".',
          );
          router.replace(`/admin/wydarzenia/dodaj/seo?id=${tripId}`);
          return;
        }
  ...
  const saveData = useCallback(async () => {
    ...
    const res = await fetch(`/api/admin/wydarzenia/${tripId}/invitation-email`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/admin/wydarzenia/dodaj/seo?id=${tripId}`);
    } catch (err) {
  ...
  const handleModalAI = useCallback(async () => {
    if (!tripContextRef.current.title) {
      toast.error("Brak danych wydarzenia. Najpierw zapisz dane podstawowe.");
      return;
    }
  ...
  const resetEmail = useCallback(async () => {
    if (!tripId) return;
    try {
      await fetch(`/api/admin/wydarzenia/${tripId}/invitation-email`, {
        method: "DELETE",
      });
      setSections([]);
      setShowAIModal(true);
    ...
    <div className="relative animate-in fade-in duration-500 pb-4">
      ...
      >
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 pt-6 mt-6 border-t border-gray-100">
          <Link
            href={`/admin/wydarzenia/dodaj/edytor-tresci${tripId ? `?id=${tripId}` : ""}`}
