...
export function TripCard({
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      const response = await fetch("/api/admin/wydarzenia/status", {
        method: "PATCH",
  ...
  const handleToggleRegistration = async () => {
    ...
    setIsUpdating(true);
    try {
      const response = await fetch("/api/admin/wydarzenia/registration", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/admin/wydarzenia/${trip.id}`, {
        method: "DELETE",
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result?.error || "Błąd usuwania");

      toast.success("Wydarzenie zostałoo usunięte.");
      setShowDeleteConfirm(false);
      // Usunięcie z listy w komponencie nadrzędnym (karta zniknie z animacją).
      onDelete?.(trip.id);
    } catch (error: any) {
      toast.error(error?.message || "Nie udało się usunąć wydarzenia.");
      setIsDeleting(false);
    }
  };
  ...
  const ActionButtons = ({ isMobile = false }: { isMobile?: boolean }) => (
    <>
      ...
      <Tooltip content="Edytuj" position="top">
        <Link href={`/admin/wydarzenia/dodaj/${trip.lastStage}?id=${trip.id}`}>
      ...
      <Tooltip
        content={
          hasBookings
            ? "Nie można usunąć — wydarzenie ma zapisane uczestniczki"
            : "Usuń"
        }
        position="top"
      >
    ...
    >
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <div className="mt-auto pt-4 border-t border-gray-100 flex flex-wrap sm:flex-nowrap items-end justify-between gap-5 w-full">
          ...
          <Link href={`/admin/wydarzenia/${trip.id}`} className="w-full sm:w-auto">
