...
// Miniatura zdjęcia wydarzenia (heroImage) z fallbackiem na ikonę.
function TripThumb({
  src,
  alt,
  className,
...
export default function AdminWydarzeniaPanel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/admin/wydarzenia?t=${Date.now()}`, {
          cache: "no-store",
          signal: ctrl.signal,
        });
  ...
  const d = useMemo(() => {
    ...
    // Nadchodzące wydarzenia: start dziś lub w przyszłości, nie archiwalne.
    ...
    if (drafts.length > 0) {
      alerts.push({
        ...
        href: "/admin/wydarzenia/lista?status=DRAFT",
      });
    }
    for (const { t, days } of upcoming) {
      ...
      if (t.status === "PUBLISHED" && days <= 14 && cap > 0 && free > 0) {
        alerts.push({
          ...
          href: `/admin/wydarzenia/${t.id}/uczestnicy`,
        });
      }
    }
  ...
  const kpis = [
    {
      ...
      href: "/admin/wydarzenia/lista?status=PUBLISHED",
    },
    {
      icon: Users,
      ...
      >
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div className="max-w-xl">
              ...
              <p className="font-montserrat text-white/70 font-medium text-[14px] mt-3 leading-relaxed">
                Twój kokpit: nadchodzące wydarzenia, sprawy wymagające uwagi i
        ...
        {!isLoading && trips.length === 0 && (
          <div className="rounded-[24px] rounded-tr-none bg-white/70 backdrop-blur-xl border border-white/60 shadow-[0_20px_55px_-36px_rgba(3,63,99,0.3)] p-12 text-center flex flex-col items-center">
            ...
            <p className="font-montserrat text-[14px] text-brand-secondary/50 mt-1 mb-5 max-w-sm">
              Nie masz jeszcze żadnego wydarzenia. Dodaj pierwszy — statystyki
        ...
        {!isLoading && trips.length > 0 && (
          <>
            ...
            {/* NAJBLIŻSZE WYDARZENIE — wizualny centerpiece ze zdjęciem */}
            {d.upcoming.length > 0 &&
              (() => {
                  ...
                  <div className="flex flex-col items-center justify-center text-center py-10 text-brand-secondary/40">
                    ...
                    <p className="font-montserrat text-[13px]">
                      {d.upcoming.length === 0
                        ...
                        : "To na razie jedyne zaplanowane wydarzenie."}
                    </p>
                  </div>
                ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <RankingCard
                ...
                icon={ChartBar}
                empty="Brak wydarzeń z ustaloną liczbą miejsc."
            ...
            >
              ...
              Zarządzaj wszystkimi wydarzenieami
              <ArrowRight
                size={15}
                weight="bold"
