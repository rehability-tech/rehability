# 🎬 Szkolenia — Moduł WYDARZENIA (admin / redakcja)

Audyt stanu + scenariusz nagrań dla panelu `/admin/wydarzenia`.
...
1. **Lista** [`/admin/wydarzenia`](./page.tsx) — strefa **„Wyróżniony na stronie głównej"** (przeciągasz wydarzenie, żeby go wypromować), filtry, karty ze statusem i liczbą zapisanych; usuwanie zablokowane, gdy są rezerwacje.
...
3. **Zarządzanie wydarzeniem** [`/admin/wydarzenia/[id]`](./[id]/page.tsx) — **realne dane** (nie mock): dashboard, lista uczestniczek, aktywność, harmonogram. Podstrony: **harmonogram** (siatka godzin/wydarzeń), **sklep** (usługi SPA), **uczestnicy** + profil pojedynczej uczestniczki, **chat** grupowy.

### 🟡 Do świadomego ominięcia / domknięcia

- [ ] **Statystyki na liście wydarzeń** — `soldSeats` i `revenuePln` są **zahardkodowane na 0** ([page.tsx:82](./page.tsx#L82), komentarz „Docelowo: połączone z rezerwacjami"). Sekcja statystyk jest pusta w renderze. → **nie pokazuj na kamerze** albo dokończ wcześniej.
...
### Przygotowanie
- Konto admina + dane demo: 1 wydarzenie opublikowane z 2–3 uczestniczkami i wpłatami, 1 wydarzenie-szkic.
- Czyste okno przeglądarki, zoom 100–110%, podświetlony kursor.

### 🎬 Odc. 2.1 — Tworzenie wydarzenia: dane + treść
`/admin/wydarzenia` → „Dodaj nowy Trip" → dane podstawowe → edytor treści z blokami (cennik, opcje rezerwacji, FAQ, mapa, wideo).
...
### 🎬 Odc. 2.4 — Zarządzanie wydarzeniem (dashboard)
`/admin/wydarzenia/[id]` → omów dashboard, uczestniczki, aktywność → **harmonogram** (dodawanie wydarzeń w siatce) → **sklep** (usługi SPA i sloty).
