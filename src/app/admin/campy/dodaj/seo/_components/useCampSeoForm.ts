...
function buildCampSummary(camp: CampRaw): string {
  let locationLabel = "";
  if (camp.location) {
    try {
      const parsed =
        typeof camp.location === "string"
          ...
          : (camp.location as { city?: string; name?: string });
      locationLabel = [parsed?.city, parsed?.name].filter(Boolean).join(" — ");
    } catch {
      locationLabel = String(camp.location);
    }
  }
  ...
  const blocksText = Array.isArray(camp.blocks)
    ? camp.blocks
        .map((b) => {
          if (!b?.type) return "";
  ...
  return [
    `Tytuł wyjazdu: ${camp.title || ""}`,
    camp.subtitle ? `Podtytuł: ${camp.subtitle}` : "",
    locationLabel ? `Lokalizacja: ${locationLabel}` : "",
    camp.startDate || camp.endDate
      ? `Termin: ${fmtDate(camp.startDate)} – ${fmtDate(camp.endDate)}`
      : "",
    Array.isArray(camp.tags) && camp.tags.length
      ? `Tagi: ${camp.tags.join(", ")}`
      : "",
    camp.description ? `Opis: ${camp.description}` : "",
...
export function useCampSeoForm(campId: string | null) {
  const router = useRouter();

  const [campTitle, setCampTitle] = useState("");
  const [campDescription, setCampDescription] = useState("");
  const [campRaw, setCampRaw] = useState<CampRaw | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchCamp = async () => {
      ...
      try {
        ...
        const data: CampRaw = await res.json();
  ...
  // Auto-uruchom analizę przy pierwszym załadowaniu campa.
  useEffect(() => {
    ...
    try {
      const res = await geminiFetch(
        "/api/admin/gemini",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            prompt: buildCampSummary(campRaw),
      ...
      if (campId) {
        try {
          const persistRes = await fetch(`/api/admin/campy/${campId}/seo`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const saveSeo = useCallback(
    async (redirect = true) => {
      ...
      try {
        ...
        if (redirect)
          router.push(`/admin/campy/dodaj/podsumowanie?id=${campId}`);
      } catch (err) {
  ...
  // Bierze aktualną analizę + bieżące pola SEO + treść campa i prosi AI o wersję,
