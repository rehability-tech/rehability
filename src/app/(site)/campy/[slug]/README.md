# `(site)/campy/[slug]` — Camp detail page & booking flow

Public-facing single camp page. Renders the camp content (saved by the admin
...
```
[slug]/
├── page.tsx                       ← server entry; fetches camp, wires providers
├── README.md                      ← you are here
└── _components/
    ├── CampDetailHero.tsx         ← hero band (title, image, dates, seats)
    ├── CampBlockRenderer.tsx      ← renders camp.blocks JSON into UI
    ...
    └── … (legacy: SingleCampHero, CampTabs, SingleCampForm, CampPageClient)
```
...
1. `page.tsx` fetches the camp from Prisma (`Camp` model, `blocks` JSON column).
2. It increments `views` and short-circuits with `notFound()` if the camp is
   not `PUBLISHED`.
3. The whole tree is wrapped in `<BookingProvider allowDuo={camp.allowBringFriend}>`.
...
The admin editor at `/admin/campy/dodaj/edytor-tresci` saves each block as:
...
| `map`            | `{}` — uses `camp.mapUrl` from the row, not the block     |
...
A modal sheet (centered on desktop, bottom-up on mobile) keeps the camp page
...
`allowDuo` is driven by `camp.allowBringFriend`. If the camp is solo-only,
...
- `createCheckoutSession` (in `src/app/actions/stripe.ts`) currently accepts
  only `{ name, email, phone, campId }`. It needs a `mode` / `friendEmail`
...
- The legacy components (`SingleCampHero`, `CampTabs`,
  `SingleCampBlockNoteRenderer`, `CampPageClient`, `SingleCampForm`) are not
  wired anymore. Sweep them once we confirm no other route imports them.
- The `map` block reads `camp.mapUrl` from the row — make sure the editor
