...
export interface CampGridCardData {
  id: string;
  title: string;
...
interface Props {
  camp: CampGridCardData;
  index?: number;
}

export default function CampGridCard({ camp, index = 0 }: Props) {
  const location = decodeLocation(camp.location);
  const freeSeats = Math.max(camp.capacity - camp.bookingsCount, 0);
  const occupancy =
    camp.capacity > 0
      ? Math.min((camp.bookingsCount / camp.capacity) * 100, 100)
      : 0;
        ...
        <div className="relative h-[240px] overflow-hidden">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-white">
            ...
            <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold bg-white/15 backdrop-blur-md border border-white/20 px-2.5 py-1 rounded-full">
              ...
              {camp.views.toLocaleString("pl-PL")}
            </span>
          </div>
        </div>
          ...
          <div className="flex items-end justify-between pt-2 border-t border-brand-secondary/5">
            <div>
              ...
              <p className="font-jakarta text-[22px] font-bold text-brand-secondary leading-none mt-1">
                {camp.price
                  ? `${camp.price.toLocaleString("pl-PL")} zł`
                  : "—"}
              </p>
            </div>
