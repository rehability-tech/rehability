 ...
 *
 * Sekwencja:
 *  1. Walidacja: token istnieje + rezerwacja należy do tego wydarzenia + nie jest anulowana.
 ...
 *  4. Wpis do feedu aktywności wydarzenia (bez push-spamu do adminów przy masowej odprawie).
 */
export async function POST(
  ...
  try {
    ...
    if (booking.tripId !== tripId) {
      return NextResponse.json(
        {
          error: `Ten bilet dotyczy innego wydarzenia${
    ...
    if (booking.status !== "FULLY_PAID") {
      return NextResponse.json(
        {
          error:
            "Bilet nieaktywny — wymagana pełna opłata za wydarzenie (opłacono tylko zadatek).",
        },
        { status: 409 },
      );
    }
    ...
    if (booking.userId) {
      await sendNotification({
        ...
        message: `Twoja obecność na wydarzeniu „${booking.trip?.title ?? ""}" została właśnie potwierdzona. Miłego pobytu!`,
        type: "BOOKING",
        link: `/panel/wydarzenia/${booking.id}`,
        push: true,
      });
    }

    // 4. Tylko wpis do feedu aktywności wydarzenia (ACTIVITY) — bez powiadamiania
    //    wszystkich adminów osobno, żeby przy odprawie 50 osób nie zalać pushy.
    await dispatchNotification({
      target: "ADMIN",
      channels: ["ACTIVITY"],
      title: "✅ Odprawa (check-in)",
      message: `${participantName} została odprawiona (wydarzenie: ${booking.trip?.title ?? ""}).`,
      type: "BOOKING",
      kind: "CHECK_IN",
      who: participantName,
      tripId,
    ...
    console.error("[POST /api/admin/wydarzenia/[id]/check-in]", error);
