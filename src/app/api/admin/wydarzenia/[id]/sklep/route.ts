...
// GET — pełny stan strony sklepu: usługi wydarzenia (ze statystykami), katalog globalny, agregaty
export async function GET(
  ...
  // Ile wydarzeń łącznie korzysta z danej usługi katalogowej (do ostrzeżenia przy edycji globalnej)
  ...
  // Mapowanie usług wydarzenia + statystyki sprzedaży
...
// POST — dodanie usług z katalogu globalnego do wydarzenia: { extraServiceIds: string[] }
...
export async function PATCH(
  ...
  if (!service) {
    return NextResponse.json(
      { error: "Usługa nie istnieje w tym wydarzeniu." },
      { status: 404 },
    );
  }
  ...
  if (!service.sourceServiceId) {
    return NextResponse.json(
      {
        error:
          "Ta usługa nie jest powiązana z katalogiem globalnym — można ją edytować tylko w tym wydarzeniu.",
      },
      { status: 400 },
    );
  }
...
// DELETE — usunięcie usługi z wydarzenia. Blokada, jeśli istnieją zamówienia.
