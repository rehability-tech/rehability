  ...
  try {
    ...
    if (isPublished && updatedTrip.bookings.length > 0) {
      ...
      const message = wasPublishedBefore
        ? `Plan dla wydarzenia „${updatedTrip.title}” został zaktualizowany. Sprawdź szczegóły w panelu.`
        : `Plan dla wydarzenia „${updatedTrip.title}” jest już dostępny w Twoim panelu. Sprawdź szczegóły i zaplanuj swój czas!`;
      ...
      await Promise.all(
        updatedTrip.bookings.map((booking) =>
          dispatchNotification({
            ...
            title,
            message,
            link: `/panel/wydarzenia/${booking.id}/harmonogram`,
            type: "SPA",
