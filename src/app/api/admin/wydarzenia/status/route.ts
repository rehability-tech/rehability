...
const statusUpdateSchema = z.object({
  ...
  status: z.enum(["PUBLISHED", "DRAFT", "ARCHIVED"], {
    message: "Nieprawidłowy status wydarzenia",
  }),
});
    ...
    if (status === "PUBLISHED" && previousStatus !== "PUBLISHED") {
      createSystemUpdate({
        ...
        description:
          ...
          "Sprawdź nowe wydarzenie w ofercie.",
        link: `/wydarzenia/${updatedCamp.id}`,
        push: true,
      }).catch((err) =>
        console.error("[wydarzenia/status] createSystemUpdate failed:", err),
      );

      // IndexNow — powiadom wyszukiwarki (Bing/Yandex/...) o nowym wydarzeniu,
      // pomijając te oznaczone jako noIndex (spójnie z sitemap.xml). notifyIndexNow
      // nigdy nie rzuca; Google dociąga wydarzenie z sitemap.xml (nie IndexNow).
      if (!updatedCamp.noIndex) {
        await notifyIndexNow(absoluteUrl(`/wydarzenia/${updatedCamp.id}`));
      }
    }
