...
export async function POST(req: Request) {
  try {
    ...
    // trwałyby już w trakcie wydarzenia).
    if (
      validatedData.registrationDeadline &&
      validatedData.registrationDeadline > validatedData.startDate
    ) {
      return NextResponse.json(
        { error: "Termin zapisów nie może być po dacie rozpoczęcia wydarzenia" },
        { status: 400 },
      );
    }
    ...
    if (id) {
      // 5a. AKTUALIZACJA
      // Pobieramy aktualny stan wydarzenia, żeby wiedzieć, czy był PUBLISHED
      ...
      // Składamy "post-update" kształt wydarzenia do walidacji kompletności.
