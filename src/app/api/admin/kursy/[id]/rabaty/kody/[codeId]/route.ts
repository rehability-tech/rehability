...
export async function PATCH(req: Request, { params }: Params) {
  ...
  const { id: slug, codeId } = await params;
  const course = await loadCourseForDiscounts(slug);
  if (!course.ok) return course.response;

  return patchPromo(req, course.owner, course.sandbox, "discountCode", codeId);
}
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  return deletePromo(course.owner, "discountCode", codeId);
}
