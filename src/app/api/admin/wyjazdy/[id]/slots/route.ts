...
export async function POST(
  ...
  // Guard: blok nie może nakładać się czasowo z istniejącym aktywnym blokiem na tym wyjeździe.
  ...
  // Whitelist: każda wybrana usługa musi należeć do tego wyjazdu i mieścić się w bloku czasowym.
  ...
  if (!isOpen && serviceCapacities.length > 0) {
    ...
    if (dbServices.length !== ids.length) {
      return NextResponse.json(
        { error: "Jedna z wybranych usług nie należy do tego wyjazdu." },
        { status: 422 },
      );
    }
