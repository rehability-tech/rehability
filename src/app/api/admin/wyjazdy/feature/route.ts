...
export async function POST(req: Request) {
  try {
    ...
    await prisma.$transaction([
      // Krok A: Zdejmujemy flagę isFeatured ze wszystkich wyjazdów
      prisma.trip.updateMany({
        data: { isFeatured: false },
      }),
      // Krok B: Jeśli podano ID, ustawiamy flagę isFeatured tylko dla tego jednego wyjazdu
      ...(id
        ? [
            prisma.trip.update({
              where: { id },
    ...
    console.error("Błąd zapisu wyróżnionego wyjazdu:", error);
    return NextResponse.json(
