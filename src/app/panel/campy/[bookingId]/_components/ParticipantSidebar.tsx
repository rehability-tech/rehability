...
export default function ParticipantSidebar({ bookingId }: Props) {
  const pathname = usePathname();
  const base = `/panel/campy/${bookingId}`;

  const items = [
    { href: base, label: "Panel", icon: House, exact: true },
    { href: `${base}/zdrowie`, label: "Karta Zdrowia", icon: Heartbeat },
    { href: `${base}/uslugi`, label: "Usługi / SPA", icon: Sparkle },
    { href: `${base}/czat`, label: "Społeczność", icon: ChatsCircle },
  ];
  ...
  return (
    <aside className="sticky top-0 left-0 h-screen w-[260px] z-40 hidden lg:flex flex-col bg-white border-r border-gray-100 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
      ...
      <nav className="flex-1 flex flex-col gap-1 overflow-y-auto px-3 pb-6">
        <span className="px-4 text-[10px] uppercase tracking-[0.2em] text-gray-400 font-bold mb-2 mt-2">
          Twój camp
        </span>
