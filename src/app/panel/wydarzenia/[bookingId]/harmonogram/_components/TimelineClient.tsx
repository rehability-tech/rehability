...
export default function TimelineClient({
  ...
  useEffect(() => {
    ...
    // Szukamy, czy "dzisiaj" jest na liscie zgrupowanych dni wydarzenia
    const foundIndex = groupedDays.findIndex(([dayKey]) => dayKey === todayKey);

    // Jeśli wydarzenie trwa teraz (dziś jest w planie), ustawiamy dzisiejszy dzień.
    // Jeśli nie (wydarzenie dopiero będzie), zostaje 0, czyli dzień pierwszy.
    if (foundIndex !== -1) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 bg-white/40 backdrop-blur-xl border border-white/60 rounded-[28px] shadow-sm text-center">
        ...
        <p className="text-[13px] font-medium text-brand-secondary/60">
          Zajrzyj tu bliżej daty wydarzenia, gdy opublikujemy pełny plan.
        </p>
      </div>
    );
  }
