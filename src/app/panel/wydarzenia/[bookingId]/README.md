...
1. **Hero z odliczaniem** (`lg:col-span-12`)
   ...
   - Powitanie po imieniu (`firstName = booking.name.split(' ')[0]`), nazwa wydarzenia, badge statusu z `STATUS_CONFIG`.
   - Countdown 4×kafelek (`days/hours/minutes/seconds`) w szkle `bg-white/15 backdrop-blur-xl border border-white/20`.
   - Po starcie wydarzenia: pigułka „Wydarzenie trwa — miłego wypoczynku!".
...
2. **Karta Bilet (Apple Wallet style)** (`lg:col-span-5`)
   ...
   - Górny pasek z gradientem `brand-secondary → brand-primary`, ID rezerwacji w prawym chipie, imię uczestniczki, nazwa wydarzenia.
...
4. **Karta Zdrowia (Krytyczna)** (drugi wiersz prawej kolumny)
   - Gdy `healthFilled === false`: cała karta dostaje neon-glow `shadow-[0_0_0_1px_rgba(244,63,94,0.12),0_18px_45px_-15px_rgba(244,63,94,0.35)]` + animowany blob `bg-rose-400/25 blur-3xl animate-pulse` + pulsujący czerwony przycisk „Uzupełnij dietę przed wydarzeniem!".
...
- `markCheckIn(qrToken)` — wywoływane z admina (`/admin/wydarzenia/.../skaner`). Po skanowaniu: `Booking.update({ isCheckedIn: true, checkedInAt: new Date() })`. Panel klientki sam to wyświetli przy następnym fetchu.
...
- Dorzucić wariant po starcie wydarzenia: ukryć countdown, podświetlić aktywny punkt agendy „dzisiaj o…".
...
3. [] Blokada biletu do momentu aż zapłacona jest kwota cała za wydarzenie
4. []
...
Opcja pozwalająca zarezerwować wydarzenie w wariancie **Duo** — uczestniczka opłaca swój zadatek i jednocześnie tworzy zaproszenie (24 h) dla wskazanej przyjaciółki, której rezerwuje miejsce.

### Przepływ end-to-end

**1. Flaga na wydarzeniu**
`Trip.allowBringFriend` — `Boolean @default(false)` w [`prisma/schema.prisma`](../../../../../prisma/schema.prisma) (linia ~61). Admin włącza ją w kreatorze wydarzenia (`app/admin/wydarzenia/dodaj/dane-podstawowe`). To jedyny przełącznik decydujący, czy wariant Duo jest w ogóle dostępny.

**2. Formularz rezerwacji (front)**
[`TripBookingForm.tsx`](../../../(site)/wydarzenia/[slug]/_components/TripBookingForm.tsx) dostaje `allowBringFriend` jako prop:
...
Powiązanie idzie po **emailu**. Gdy zaproszona loguje się tym samym adresem, jej booking dopasowuje się po mailu — [`resume-payment`](../../../api/panel/wydarzenia/resume-payment/route.ts) używa `where: { id, email: session.user.email }` i **dopuszcza status `PENDING_INVITATION`**, traktując go jak zadatek. Panel [`app/panel/wydarzenia/page.tsx`](../page.tsx) pokazuje go w sekcji „do opłaty". Każdy booking ma `amountTotal` = pełna cena, więc przyjaciółka też płaci najpierw swój zadatek, a potem resztę.
...
3. **Strona akceptacji** — [`app/zaproszenie/[token]/page.tsx`](../../../zaproszenie) (Server Component). Obsługuje stany: nieprawidłowy token / wygasłe / już przejęte (przez Ciebie → CTA do panelu, lub przez kogoś innego) / właściwy (szczegóły wydarzenia + przycisk).
4. **Przejęcie** — niezalogowana loguje się Google z `callbackUrl` na ten sam link; po powrocie `POST /api/zaproszenia/accept` ustawia `userId` **oraz** `email` na zalogowaną (panel matchuje po emailu), status zostaje `PENDING_INVITATION`. Redirect do `/panel/wydarzenia`, gdzie gościni opłaca swój zadatek przez istniejący `resume-payment`.
