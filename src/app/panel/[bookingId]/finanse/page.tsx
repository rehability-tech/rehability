...
export default async function FinansePage({ params }: Props) {
  ...
  const booking = await prisma.booking.findUnique({
    where: { id: bookingId },
    include: {
      camp: { select: { title: true, price: true, deposit: true } },
      serviceOrders: {
  ...
  const campPrice = Number(booking.camp.price);
  const campDeposit = Number(booking.camp.deposit);
